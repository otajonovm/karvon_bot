...
const { fetchActiveDrivers } = require('./drivers');
const { routeMatchesOrder } = require('./routeMatch');

async function findDriversForBroker({ truck_type, from_region, to_region }) {
  const order = { truck_type, car_type: truck_type, from_region, to_region };
  const matched = (await fetchActiveDrivers()).filter((d) => routeMatchesOrder(d, order));
