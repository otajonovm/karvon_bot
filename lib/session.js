...
function loadSession() {
  ...
  if (plain) {
    if (looksLikeGramJsSession(plain)) return plain;

    // Ko'p hollarda base64 qiymatni xato bilan TELEGRAM_SESSION ga qo'yishadi —
    // avtomat aniqlab decode qilamiz (GramJS plain session talab qiladi).
    const decoded = decodeSessionB64(plain);
    if (decoded && looksLikeGramJsSession(decoded)) {
      console.warn(
        '[session] TELEGRAM_SESSION ichida base64 topildi — avtomat decode qilindi'
      );
      console.warn(
        '[session] Tavsiya: bu qiymatni TELEGRAM_SESSION_B64 kalitiga ko\'chiring'
      );
      return decoded;
    }
    return plain;
