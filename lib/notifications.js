...
const { ORDER_STATUS, DRIVER_STATUS, DRIVER_CITIES } = require('../config/constants');
...
/**
 * Find active drivers:
 * 1. Eski format — yo'nalish juftligi: "Toshkent-Vodiy"
 * 2. Yangi format — shahar nomi: "Farg'ona", "Toshkent" (preferred_route = single city)
 * 3. Fallback — faqat mashina turi bo'yicha
 */
async function findMatchingDrivers(order) {
  ...
  // 1️⃣ Eski format: yo'nalish juftligi
  const byRoute = activeOnly(
  ...
  if (byRoute.length > 0) {
    return { drivers: byRoute, route, reverse, matchType: 'route' };
  }

  // 2️⃣ Yangi format: buyurtmaning from/to regioniga mos keladigan shaharlar
  const matchingCityLabels = DRIVER_CITIES
    .filter((c) => c.region === order.to_region || c.region === order.from_region)
    .map((c) => c.label);

  if (matchingCityLabels.length > 0) {
    const byCity = activeOnly(
      await fetchDrivers((supabase, cols) =>
        supabase
          .from('drivers')
          .select(cols)
          .eq('car_type', order.car_type)
          .in('preferred_route', matchingCityLabels)
      )
    );

    if (byCity.length > 0) {
      return { drivers: byCity, route, reverse, matchType: 'city' };
    }
  }

  // 3️⃣ Fallback: faqat mashina turi
