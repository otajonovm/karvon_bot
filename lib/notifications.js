...
function orderActionKeyboard(order, { includeTel = true } = {}) {
  const rows = [];
  const contactRow = [];
...
async function sendDriverPush(telegram, driver, order) {
  ...
  try {
    ...
    if (!/tel:|Wrong port number|inline keyboard/i.test(err.message)) {
      throw err;
    }

    console.warn(`[notify] ${driver.user_id}: tugma xatosi, tel siz qayta urinilmoqda`);
    return telegram.sendMessage(driver.user_id, text, {
      ...opts,
      ...orderActionKeyboard(order, { includeTel: false }),
    });
  }
}
