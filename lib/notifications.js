...
/** Muddati o'tgan yuk ekanligini tekshiradi (in-memory, DB siz) */
function isOrderExpiredLocal(order) {
  if (!order) return true;
  if (order.status === 'expired') return true;
  if (order.expires_at && new Date(order.expires_at) < new Date()) return true;
  return false;
}
...
function orderActionKeyboard(order, { includeTel = false, includeDm = true } = {}) {
  const rows = [];
  const contactRow = [];
...
async function notifyMatchingDrivers(telegram, order) {
  // Expired yukni haydovchilarga yubormaylik
  if (isOrderExpiredLocal(order)) {
    console.log(`[notify] Skip expired order ${order?.id}`);
    return;
  }
  ...
  if (notifyAllDrivers()) {
    console.log(
      `[notify] Barcha faol haydovchilarga: ${order.from_region}→${order.to_region} (${drivers.length} ta)`
    );
  }
...
async function pushRecentMatchingOrders(
  ...
  if (!driver?.user_id || !driver.from_region || !driver.to_region) {
    console.warn(
      `[notify] recent push skip: profil to'liq emas (user=${driver?.user_id}, ` +
  ...
  if (matches.length === 0) {
    console.log(
      `[notify] recent push 0: ${driver.user_id} ${driver.from_region}→${driver.to_region} ` +
        `(${driver.car_type || driver.truck_type}), pool=${pool.length}, window=${sinceMinutes}m`
    );
    return 0;
  }
  ...
  if (sent > 0) {
    console.log(
      `[notify] Catch-up: ${sent} ta mos yuk → ${driver.user_id} ` +
        `(${driver.from_region}→${driver.to_region})`
    );
  }
  return sent;
