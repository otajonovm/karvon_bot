...
async function pushRecentMatchingOrders(telegram, driver, { limit = 8 } = {}) {
  ...
  const { data: orders, error } = await supabase
    .from('orders')
    .select('id,from_region,to_region,cargo_details,car_type,truck_type,price_uzs,status,created_at,sender_username,sender_telegram_id,broker_user_id')
...
async function acceptOrder(orderId, driverId) {
  ...
  const { data: order, error: fetchError } = await supabase
    .from('orders')
    .select('id,from_region,to_region,cargo_details,car_type,truck_type,price_uzs,status,created_at,sender_username,sender_telegram_id,broker_user_id,notes')
    .eq('id', orderId)
    .single();
  ...
  if (updateError || !updated) {
    ...
    const { data: current } = await supabase.from('orders').select('id,from_region,to_region,cargo_details,car_type,truck_type,price_uzs,status,created_at,sender_username,sender_telegram_id,broker_user_id').eq('id', orderId).single();
