...
const { botStartUrl } = require('./botLinks');
...
const EDUCATION_TEXT =
  ...
  `📦 E'lon: ${BOT_HANDLE} → [ Yuk Joylash ]\n` +
  `🚚 Yuk olish: botda haydovchi sifatida ro'yxatdan o'ting.\n` +
  `Mijoz raqami faqat ro'yxatdan o'tgan haydovchiga ochiladi.`;

const EDUCATION_KEYBOARD = {
  inline_keyboard: [
    [{ text: "🚚 Haydovchi bo'lib yuk olish", url: botStartUrl('driver') }],
    [{ text: "📦 E'lon berish", url: BOT_URL }],
  ],
};
  ...
  try {
    const sent = await telegram.sendMessage(chatId, text, {
      ...
      ...Markup.inlineKeyboard([
        [Markup.button.url("🚚 Haydovchi bo'lib yuk olish", botStartUrl('driver'))],
        [Markup.button.url("📦 E'lon berish", BOT_URL)],
      ]),
    });
