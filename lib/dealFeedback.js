...
async function sendFeedbackMessage(telegram, row) {
  ...
  const { data: order } = await supabase
    .from('orders')
    .select('id, from_region, to_region, status, taken_by, source, created_at, expires_at')
    .eq('id', row.order_id)
    .maybeSingle();

  // Eski push/catch-up yozuvlari ham navbatda qolishi mumkin.
  // Feedback faqat shu haydovchi guruhdagi yukni olganidan keyin yuboriladi.
  if (
    !order ||
    order.status !== 'taken' ||
    String(order.taken_by) !== String(row.driver_id)
  ) {
