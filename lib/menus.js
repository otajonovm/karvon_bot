...
function driverRoutePresetKeyboard() {
  return Markup.inlineKeyboard([
    [Markup.button.callback('🌍 Har qanday yo\'nalish (Barcha viloyatlar)', 'drv_rt_all')],
    ...
    [Markup.button.callback('🎯 Aniq viloyatdan chiqish (O\'zim tanlayman)', 'drv_rt_custom')],
  ]);
}

function cabinetInlineKeyboard() {
  return Markup.inlineKeyboard([
    [Markup.button.callback('🔄 Marshrut / Joylashuvni o\'zgartirish', 'change_route')],
    [
      Markup.button.callback('🟢 Bo\'shman', 'driver_set_active'),
      Markup.button.callback('🔴 Bandman', 'driver_set_busy'),
    ],
  ]);
}
