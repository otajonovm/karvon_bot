...
function driverMatchesOrder(driver, order) {
  ...
  if (regionsTextMatch(from, driver.current_location)) return true;

  const routes = parsePreferredRoutes(driver.preferred_routes);
  if (routes.some((r) => r !== ALL_ROUTES && r !== ANY_DEST && regionsTextMatch(from, r))) {
    return true;
  }
  if (driver.from_region && regionsTextMatch(from, driver.from_region)) return true;
  return false;
}
