...
const { regionsTextMatch, carTypesMatch } = require('./routeMatch');
...
/**
 * Push: ALL_ROUTES → mashina turi.
 * Aniq hudud → order.from_region haydovchi hududlariga mos.
 */
function driverMatchesOrder(driver, order) {
  ...
  const origins = matchOrigins(driver);
  if (origins.length === 0) {
    return regionsTextMatch(order.from_region, driver.from_region);
  }
  return origins.some((o) => regionsTextMatch(order.from_region, o));
}
