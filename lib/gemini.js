...
const { parseWithRegex, extractRoute } = require('./regexParser');
...
const AI_PROMPT_STRICT =
  ...
  "Map districts: Qo'qon/Marg'ilon/Rishton/Oltiariq/Quva/Bog'dod/Uchko'prik→Farg'ona; " +
  "Asaka/Shahrixon/Xo'jaobod/Bo'z/Marhamat/Qorasuv→Andijon; " +
  "Chust/Pop/Kosonsoy/To'raqo'rg'on/Uychi→Namangan; " +
  "Kattaqo'rg'on/Urgut/Toyloq/Oqdaryo/Pastdarg'om→Samarqand; " +
  "Qarshi/Shahrisabz/Kitob/G'uzor/Koson/Nishon→Qashqadaryo; " +
  "Termiz/Denov/Sherobod/Sho'rchi/Boysun→Surxondaryo. " +
  "Cars:Labo/Damas,Gazel,Isuzu,Fura. Phone:+998. Spam:{\"error\":\"spam\"}";

const AI_PROMPT_RELAXED =
  "JSON only: from_region,to_region,car_type,cargo_details,phone_number. " +
  "Regions:Toshkent,Farg'ona,Andijon,Namangan,Samarqand,Buxoro,Qashqadaryo,Surxondaryo. " +
  "Districts MUST map to province: Qo'qon/Kokand/Marg'ilon/Rishton/Quva→Farg'ona; " +
  "Asaka/Shahrixon/Marhamat→Andijon; Chust/Pop/Kosonsoy→Namangan; " +
  "Urgut/Kattaqo'rg'on→Samarqand; Qarshi/G'uzor/Koson→Qashqadaryo; Termiz/Denov→Surxondaryo. " +
  "Navoiy/Jizzax/Sirdaryo→Qashqadaryo. " +
  "Cars:Labo/Damas,Gazel,Isuzu,Fura,chakman,kamaz→Fura. Always extract phone +998 if present.";
...
async function parseCargoMessage(text) {
  ...
  // Regex to'liq tutmadi — marshrut+telefon bor bo'lsa AI siz tez yo'l
  const route = extractRoute(text);
  const phone = extractPhoneFromText(text);
  if (route && phone) {
    const quick = normalizeParsedOrder({
      from_region: route.from_region,
      to_region: route.to_region,
      car_type: 'Fura',
      cargo_details: text.slice(0, 300),
      phone_number: phone,
    });
    if (quick) {
      aiStats.regex++;
